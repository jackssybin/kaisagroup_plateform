...
//	}
	 ...
	 * @return ***
	 */
	public static  String kickstarOnCardNum2(String cardNum) {
		if (null == cardNum || "".equals(cardNum)) {
			return cardNum;
		}
		int starNum = cardNum.length() - 4;
		List starLi = new ArrayList();
		int Fstar = starNum/4;
		int Ostar = starNum % 4;
		String showNum = "";
		...
		for(int i = 0 ; i<Ostar ; i++){
			
			starLi.add("*");
		}
		String tmp = "";
		for(int i = starLi.size()-1 ; i >= 0; i--){
			tmp = starLi.get(i) + tmp;
		}
		if(Ostar == 0){
			showNum = "$1$2";	
		}else{
		...
		cardNum = cardNum.replaceAll("\\d{0,"+starNum+"}(\\d{1})(\\d{3})", tmp+showNum);
		return cardNum;
	}
	...
	public static  String kickstarOnIdNum2(String idNum) {
		if (null == idNum || "".equals(idNum)) {
			return idNum;
		}
		//realName = realName.replaceAll("([\u4e00-\u9fa5 a-z A-Z 1-9]{1})[\u4e00-\u9fa5 a-z A-Z 1-9]*", "$1*");
		idNum = idNum.replaceAll("\\d{0,14}([a-z A-Z 0-9]{4})", "***************$1");
		return idNum;
	}
